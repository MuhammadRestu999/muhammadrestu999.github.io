...
function copyText(){document.getElementById('hasilnye').select()
document.execCommand('copy')}
function copyText2(){document.getElementById('hasilnye_delete').select()
document.execCommand('copy')}
function alert_gagal(result){const wrapper=document.createElement('div')
...
function alert_success(result){const wrapper=document.createElement('div')
...
hasil.append(wrapper)}
$('#form-input').submit(function(e){e.preventDefault()
btn_loading.classList.toggle('d-none');btn_kirim.classList.toggle('d-none');$.ajax({type:"POST",url:base_url+'/create',timeout:30000,data:{url:$('#url').val(),costum:$('#costum').val()},dataType:"json",success:function(response){btn_loading.classList.toggle('d-none');btn_kirim.classList.toggle('d-none');alert_success(response)
form.reset();console.log('Success!',response);},error:function(err){console.log(err.responseJSON)
btn_loading.classList.toggle('d-none');btn_kirim.classList.toggle('d-none');alert_gagal(err.responseJSON)}})})